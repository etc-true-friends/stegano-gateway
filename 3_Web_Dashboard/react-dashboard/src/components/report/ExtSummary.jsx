...
export default function ExtSummary({ logs }) {
  const extCounts = {};
  logs.forEach(log => {
  ...
  const top = Object.entries(extCounts).sort((a, b) => b[1] - a[1]).slice(0, 3);
    ...
    <div className="stat-grid">
      {top.map(([ext, count], i) => (
        <div key={ext} className="stat-card" style={{ borderTop: `3px solid ${COLORS[i % COLORS.length]}` }}>
          ...
          <div style={{ marginTop: 8, fontSize: 12, color: '#64748b' }}>{count}건</div>
        </div>
      ))}
    </div>
  );
