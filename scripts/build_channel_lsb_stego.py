...
def embed_channel_lsb(rgb, payload_ratio, channel, seed):
    ...
    arr = rgb.copy()
    c = int(rng.integers(0, 3)) if channel == "random" else {"r": 0, "g": 1, "b": 2}[channel]
...
def main():
    ...
    parser.add_argument("--payload_ratio_max", type=float, default=0.40)
    ...
    for idx, path in enumerate(images):
        try:
            ...
            stego = embed_channel_lsb(rgb, payload_ratio, args.channel, args.seed + idx)
            save_rgb(stego, output_dir / f"{path.stem}.png")
            ok += 1
        except Exception as exc:
            skipped += 1
            print(f"[SKIP] {path.name}: {exc}")
    print(f"done: {ok} channel LSB stego images saved to {output_dir} / skipped: {skipped}")
